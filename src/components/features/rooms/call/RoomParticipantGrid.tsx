...
/**
 * Session 3 — ported verbatim from Portal's `ParticipantGrid.tsx`.
 *
 * This is a *fork*, not a shared component: Anomaly's `PublicCallPanel`
 * imports the existing `@/components/features/calls/ParticipantGrid`
 * directly (styled/sized for its own docked layout), so editing that file
 * would change Anomaly's public lobby call UI. This copy is scoped to
 * plain chat/call rooms only (mounted by `RoomCallOverlay`) and never
 * touches, and is never touched by, anything under `features/anomaly/`.
 *
 * Shared, non-visual dependencies (`ParticipantCard`, `useCallStore`,
 * `useCalls`, `useRoomCallContext`) are reused unmodified — they're plain
 * data/state, not layout, so there's nothing Anomaly-specific to protect
 * there.
 */
export const RoomParticipantGrid = () => {
  const {
    callId,
    actualRoomId,
    remoteStreams,
  ...
  const { activeCalls, participantProfiles } =
    useRoomCallContext() || useCalls(actualRoomId || "");
