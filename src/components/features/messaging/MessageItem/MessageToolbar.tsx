...
export const MessageToolbar = ({
  ...
  return (
    <div
      className={`absolute top-1/2 -translate-y-1/2 z-[70] transition-opacity flex items-center gap-0.5 bg-theme-base border border-theme-border rounded-[7px] p-0.5 shadow-lg group-hover/row:opacity-100 ${isCurrentUser ? "right-full mr-4" : "left-full ml-4"} ${menuOpen || hoverCardOpen ? "opacity-100" : "opacity-0"}`}
    >
      <HoverCard.Root
        openDelay={100}
        closeDelay={300}
        ...
        >
          ...
          <DropdownMenu.Portal>
            <DropdownMenu.Content
              sideOffset={8}
              align="end"
              className="w-auto min-w-[120px] pointer-events-auto bg-theme-base border border-theme-border rounded-md z-[100] animate-in fade-in duration-100 outline-none"
            >
              {content && (
                <DropdownMenu.Item
                  onClick={handleCopy}
                  className="px-3 py-2 text-xs text-gray-300 hover:bg-theme-hover flex items-center rounded-t-sm gap-2 cursor-pointer outline-none"
                >
              ...
              {isCurrentUser && (
                <DropdownMenu.Item
                  ...
                  className="px-3 py-2 text-xs text-red-300 hover:bg-theme-hover flex items-center rounded-b-sm gap-2 cursor-pointer outline-none"
                >
